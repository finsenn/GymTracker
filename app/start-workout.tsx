...
import { View, Text, StyleSheet, SafeAreaView, ScrollView, TouchableOpacity, Modal, Platform, Animated } from 'react-native';
...
export default function StartWorkoutModal() {
  ...
  // Animation value for the main action button
  ...
  // *** THE FIX IS HERE ***
  // This effect controls whether the user can swipe down to dismiss the modal.
  useEffect(() => {
    const isSelectionState = ['dayType', 'subCategory', 'exerciseList'].includes(screenState);
    navigation.setOptions({
      gestureEnabled: isSelectionState, // Enable gesture only during selection
    });
  ...
  const handleWorkPress = async () => {
    ...
    setWorkTimer(0); setRestTimer(180);
  ...
  const handleFinishWorkout = () => {
    ...
    if(finalLog.length > 0) {
        ...
        addWorkoutToHistory({ date: dateString, totalTime: sessionTimer, exercises: finalLog });
    }
  ...
  const renderContent = () => {
    if (isSelectionState) {
        if (screenState === 'dayType') {
          return (
            <ScrollView contentContainerStyle={styles.scrollContainer}><Text style={styles.title}>What day is it?</Text>
              <TouchableOpacity style={styles.choiceButton} onPress={() => selectAndGoTo('subCategory')('ppl')}><Text style={styles.choiceText}>PPL</Text></TouchableOpacity>
              <TouchableOpacity style={styles.choiceButton} onPress={() => selectAndGoTo('subCategory')('upperlower')}><Text style={styles.choiceText}>Upper / Lower</Text></TouchableOpacity>
              <TouchableOpacity style={styles.choiceButton} onPress={() => selectAndGoTo('exerciseList')('fullbody')}><Text style={styles.choiceText}>Full Body</Text></TouchableOpacity>
              <TouchableOpacity style={styles.choiceButton} onPress={() => selectAndGoTo('subCategory')('bro')}><Text style={styles.choiceText}>Bro Split</Text></TouchableOpacity>
            </ScrollView>
          );
        }
        if (screenState === 'subCategory') {
          ...
          return (
            <ScrollView contentContainerStyle={styles.scrollContainer}><Text style={styles.title}>{dayCategory.toUpperCase()}</Text>
              {categories.map(cat => (<TouchableOpacity key={cat} style={styles.choiceButton} onPress={() => selectAndGoTo('exerciseList')(cat)}><Text style={styles.choiceText}>{cat}</Text></TouchableOpacity>))}
            </ScrollView>
          );
        }
        if (screenState === 'exerciseList') {
          return (
            <View style={{flex: 1}}><Text style={styles.title}>Choose your poison</Text>
              <ScrollView contentContainerStyle={styles.scrollContainer}>
                {getWorkoutList().map(ex => (<TouchableOpacity key={ex} style={styles.choiceButton} onPress={() => handleExerciseSelect(ex)}><Text style={styles.choiceText}>{ex}</Text></TouchableOpacity>))}
              </ScrollView>
            </View>
          );
        }
      ...
      return (
        <View style={styles.centerContainer}>
          <Text style={styles.sessionTimerText}>Session Paused</Text>
    ...
    } else if (screenState === 'working') {
        return (
            <View style={styles.centerContainer}>
                <Text style={styles.sessionTimerText}>Session: {formatTime(sessionTimer)}</Text>
                <Text style={styles.exerciseText}>{currentExercise}</Text>
                <Text style={styles.workTimerText}>Working: {formatTime(workTimer)}</Text>
                <Animated.View style={animatedButtonStyle}><TouchableOpacity style={styles.mainActionButtonRest} onPress={handleRestPress}><Text style={styles.mainActionButtonText}>REST</Text></TouchableOpacity></Animated.View>
                <TouchableOpacity onPress={() => setScreenState('exerciseList')}><Text style={styles.linkText}>Change Workout</Text></TouchableOpacity>
                <View style={styles.finishButtonContainer}><TouchableOpacity style={styles.finishButton} onPress={handleFinishWorkout}><Text style={styles.finishButtonText}>FINISH</Text></TouchableOpacity></View>
            </View>
        );
    } else if (screenState === 'resting') {
        const isOvertime = restTimer === 0;
        const overtimeValue = sessionTimer - workTimer - (workoutLog[currentExercise]?.restTime || 0) - 180;
        return (
            <View style={styles.centerContainer}>
                <Text style={styles.sessionTimerText}>Session: {formatTime(sessionTimer)}</Text>
                <Text style={[styles.timerLabel, isOvertime && { color: Colors.light.warning }]}>{isOvertime ? "REST OVERTIME" : "RESTING"}</Text>
                <Text style={[styles.timer, isOvertime && { color: Colors.light.warning }]}>{isOvertime ? `+${formatTime(overtimeValue > 0 ? overtimeValue : 0)}` : formatTime(restTimer)}</Text>
                <Animated.View style={animatedButtonStyle}><TouchableOpacity style={styles.mainActionButton} onPress={handleWorkPress}><Text style={styles.mainActionButtonText}>WORK</Text></TouchableOpacity></Animated.View>
                <TouchableOpacity onPress={() => setScreenState('exerciseList')}><Text style={styles.linkText}>Next Workout</Text></TouchableOpacity>
                <View style={styles.finishButtonContainer}><TouchableOpacity style={styles.finishButton} onPress={handleFinishWorkout}><Text style={styles.finishButtonText}>FINISH</Text></TouchableOpacity></View>
            </View>
        );
    }
      ...
      <View style={styles.header}>
        {isSelectionState && (
          <TouchableOpacity onPress={() => router.back()}>
            <FontAwesome name="close" size={24} color={Colors.light.text} />
          </TouchableOpacity>
        )}
      </View>
